...
export default function PlanChildEducation() {
  ...
  useEffect(() => {
    const calculateEducationPlan = () => {
      ...
      const futureValueCalculated = investmentAmount * Math.pow(1 + inflationRate, years + remainingMonths / 12);
      ...
      const lumpsum = futureValueCalculated / Math.pow(1 + annualRate, years + remainingMonths / 12);
  ...
  }, [childAge, investmentAmount, withdrawalAge, growthRate, investmentType]);
          ...
          {investmentType === 'monthly' ? (
            <div>
              ...
              <div className="bg-white p-4 rounded-lg shadow-md text-center mt-4">
                ...
                <p className="text-2xl font-bold">{`₹ ${(futureValue - sipAmount * (numberOfYears * 12 + numberOfMonths)).toFixed(2)}`}</p>
              </div>
            </div>
          ) : (
            <div>
